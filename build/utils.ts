...
export function getPackageFileList(rootDir: string): Array<string> {
	const filepathListWithoutFilter = fs.readdirSync(rootDir, {
		encoding: "utf-8",
		recursive: true,
	});
	const edaignoreListWithoutResolve = multiLineStrToArray(
		fs.readFileSync(path.join(rootDir, ".edaignore"), {
			encoding: "utf-8",
		}),
	);
	...
	for (const edaignoreLine of edaignoreListWithoutResolve) {
		if (edaignoreLine.endsWith("/") || edaignoreLine.endsWith("\\")) {
			edaignoreList.push(
				edaignoreLine.slice(0, edaignoreLine.length - 1),
			);
		} else {
	...
	const filepathListWithoutResolve = edaignore.filter(
		filepathListWithoutFilter,
	);
