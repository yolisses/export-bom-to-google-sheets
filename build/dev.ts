...
function resolveExtensionConfig() {
	const extensionConfig = { ...rawExtensionConfig } as Record<
		string,
		unknown
	>;
	if (!testUuid(extensionConfig.uuid as string | undefined)) {
		delete extensionConfig.default;
		extensionConfig.uuid = fixUuid(
			extensionConfig.uuid as string | undefined,
		);
		fs.writeJsonSync(
...
async function broadcastEext(wss: WebSocketServer): Promise<void> {
	...
	console.log(
		`[Dev Mode] Pushed update to ${clientCount} client(s): ${EEXT_FILENAME}`,
	);
}
...
async function main() {
	...
	console.log(
		`[Dev Mode] WebSocket server started: ws://localhost:${WEBSOCKET_PORT}`,
	);
	...
	wss.on("connection", async (ws) => {
		...
		try {
			...
			ws.send(message);
			console.log(
				`[Dev Mode] Sent current version to new client: ${EEXT_FILENAME}`,
			);
		} catch (err) {
			...
			build.onEnd((result) => {
				if (result.errors.length === 0) {
					...
					buildTimeout = setTimeout(async () => {
						console.log(
							"[Dev Mode] Change detected, repackaging...",
						);
						try {
